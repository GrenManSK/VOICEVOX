...
def main(stdscr):
    ...
    def string(x, y, content, move=0, refresh=True):
        ...
        try:
            ...
            error.append(
                [x, y, content, 'Please resize the window, and try again'])
        window[x] = content
        current_row += move
        if refresh:
            stdscr.refresh()
    ...
    def clear():
        ...
        stdscr.clear()
        window = {}
        string(curses.LINES-3, int(curses.COLS/3), 'Using character: ' +
               name + ' with code: ' + current + '    ')
        string(curses.LINES-2, int(curses.COLS/3), 'Using VOICEVOX')
        string(
            curses.LINES-1, int(curses.COLS/3), 'Check for additional information at https://github.com/VOICEVOX/voicevox_engine')
        ...
        if len(error) == 0:
            ...
            string(int((curses.LINES - 5)/2 + 1), int(curses.COLS/3),
                   'Japanese translation history:')
            for i in range(curses.LINES - 3):
                string(i, int(curses.COLS/3) - 1, '|')
            string((curses.LINES - 5) + 1, int(curses.COLS/3) -
                   1, '-'*(int(curses.COLS/3)*2 + 1))
            string(int((curses.LINES - 5)/2),
                   int(curses.COLS/3), '-'*(int(curses.COLS/3)*2))
            for times, text in enumerate(history):
                string(times + 1, int(curses.COLS/3), text)
            for times, text in enumerate(history_jap):
                string(times + int((curses.LINES - 5)/2 + 3),
                       int(curses.COLS/3), text)
        else:
            ...
            if len(history_jap) > (curses.LINES - 5)/3:
                history_jap.pop(0)
            string(0, int(curses.COLS/3), 'English history:')
            string(int((curses.LINES - 5)/3 + 1), int(curses.COLS/3),
                   'Japanese translation history:')
            string(int(((curses.LINES - 5)/3)*2 + 3),
                   int(curses.COLS/3), 'Error log:')
            for times, i in enumerate(error):
                string(int(((curses.LINES - 5)/3)*2 + 4) + times, int(curses.COLS/3),
                       f'Error occured at line {i[0]} column {i[1]}: {i[3]}')
            for i in range(curses.LINES - 3):
                string(i, int(curses.COLS/3) - 1, '|')
            string((curses.LINES - 5) + 1, int(curses.COLS/3) -
                   1, '-'*(int(curses.COLS/3)*2 + 1))
            string(int((curses.LINES - 5)/3 + 1),
                   int(curses.COLS/3), '-'*(int(curses.COLS/3)*2))
            string(int((curses.LINES - 5)/3 + 1),
                   int(curses.COLS/3), '-'*(int(curses.COLS/3)*2))
            string(int(((curses.LINES - 5)/3)*2 + 2),
                   int(curses.COLS/3), '-'*(int(curses.COLS/3)*2))
            for times, text in enumerate(history):
                string(times + 1, int(curses.COLS/3), text)
            for times, text in enumerate(history_jap):
                string(times + int((curses.LINES - 5)/3 + 3),
                       int(curses.COLS/3), text)
    ...
    for i in range(audio.get_device_count()):
        ...
        string(current_row + 2, 0,
            f"Current: {info['name']} at index {info['index']}     ")
        sleep(0.1)
    ...
    name = os.path.abspath('VOICEVOX/VOICEVOX/run.exe')
    ...
    def speak():
        ...
        vboxapp.run(text=translation, speaker=current,
    ...
    def play_audio():
        ...
        while channel.get_busy():
            sleep(0.1)
        string(row, 0,
               'Playing audio through in device CABLE Input (VB-Audio Virtual Cable) DONE', 2)
    ...
    def record_audio():
        ...
        string(
            current_row, 0, 'Opening stream in output device...')
        ...
        string(
            current_row, 0, 'Opening stream in output device DONE', 1)

        string(
            current_row, 0, 'Playing sound in output device...')
        chunk_size = 1024
        ...
        string(
            current_row, 0, 'Playing sound in output device DONE', 2)

        string(
            current_row, 0, 'Closing stream in output device...')
        ...
        string(
            current_row, 0, 'Closing stream in output device DONE', 1)
    ...
    def change_char(key, to: str):
        ...
        if key:
            ...
            name = to
            string(curses.LINES-3, int(curses.COLS/3),
                   'Using character: ' + name + ' with code: ' + current + ' '*(curses.COLS - len(current) - len(name) - 29))
